 ...
 *
 * +-----------+                +-----------+        訂閱       +-------------------------+
 * |           |                |           |  <============>  | Subscriber (Observer_1) |
 * |           |     發佈消息    |           |        觸發       +-------------------------+
 * | Publisher | -------------> |  dispatch |        ...
 * | (Subject) |                |   center  |        訂閱       +------------------------+
 * |           |                |           |  <============>  | Subscriber (Observer_n)|
 * |           |                |           |        觸發       +------------------------+
 * +-----------+                +-----------+
 *
 *
...
class DispatchCenter implements IDispatchCenter {
  ...
  unsubscribeTopic(topic: string, subscriber: ISubscriber){
    const index = this.dispatchCenter[topic].findIndex((suber) => suber.name === subscriber.name);
    this.dispatchCenter[topic].splice(index,1);  
  }
  publish(topic: string, message: string){
    this.dispatchCenter[topic]?.forEach((Subscriber) => Subscriber.update(topic , message));
  }
}
class Publisher implements IPublisher {
  ...
  ["constructor"](_name: string, _dispatchCenter: IDispatchCenter): void {
